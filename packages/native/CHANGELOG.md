...
- b9a3d5f: Add a new `Screen` component in `view` with theme background by default and optional `padding` support.
...
- 2bb61bb: Add consistent `fullWidth` behavior across native view layout components by applying `flexGrow: 1` and `flexShrink: 1`, add `fullWidth` support to `Surface`, and introduce a new `Center` view component in `@xaui/native/view`.
...
- b948048: fix(native): fix dark mode background colors on Button, FAB, Badge and SegmentButton
...
- 7495b04: Add InputTrigger, Picker and ColorPicker components
...
- 8583808: Add a new `TextSpan` typography primitive to compose grouped text content and share inherited text styles (`color`, `fontWeight`, `fontStyle`, `textTransform`) with nested `Typography`.
...
- b0e37dd: feat(native): improve PieChart and add HeatmapChart component
